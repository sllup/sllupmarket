...
app = FastAPI(title="Engajamento API v6.1.1 (LOCAL dbt)")
...
def apply_user_header_map(header: List[str], header_map: Optional[Dict[str,str]]):
    ...
    for req_col, provided_name in header_map.items():
        ...
        if idx is None: raise HTTPException(status_code=400, detail=f"header_map aponta '%s' que não existe no CSV" % provided_name)
        out[req_col]=idx
    return out
