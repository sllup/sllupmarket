...
st.title("📊 Engajamento B2B – v3 (Importar por URL)")
...
# -------------------------
# 🌐 Importar CSV por URL (server-side ingest via API)
# -------------------------
tab4, = st.tabs(["Importar por URL"])
...
with tab4:
    ...
    if run:
        if not api_base or not csv_url:
            ...
            try:
                payload = {"url": csv_url, "mode": "full" if mode.startswith("Full") else "append"}
                resp = requests.post(api_base.rstrip("/") + "/ingest/url", json=payload, timeout=900)
                if resp.status_code == 200:
                    data = resp.json()
                    if data.get("ok"):
                        st.success(f"Ingest concluído ✅ Linhas ~{data.get('rows')}")
